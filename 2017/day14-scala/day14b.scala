...
object day14b {
  ...
  def main(args: Array[String]): Unit = {
        ...
        var arr = Array.ofDim[Int](128,128)
        ...
        for(x <- 0 to 127 ){
            ...
            for (y<-0 to 127){
                arr(x)(y)=Integer.parseInt(bs.charAt(y).toString,10);
            }
    ...
    def region(arr: Array[Array[Int]], i:Int, j:Int): Unit ={
        arr(i)(j)=2;

        if (i>0 && arr(i-1)(j)==1) {
            region(arr,i-1,j)
        }
        if (i<127 && arr(i+1)(j)==1) {
            region(arr,i+1,j)
        }
        if (j>0 && arr(i)(j-1)==1) {
            region(arr,i,j-1)
        }
        if (j<127 && arr(i)(j+1)==1) {
            region(arr,i,j+1)
        }
    } 
